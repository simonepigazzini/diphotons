...
./bkg_bias.py --analyze-bias --bias-files $inputdir/bias_study_toys_from_mc_unbinned_pow4/EBEB/toys.root --bias-labels pow4 --bias-files $inputdir/bias_study_toys_from_mc_unbinned_invpow/EBEB/toys.root --bias-labels invpow --bias-files $inputdir/bias_study_toys_from_mc_unbinned_invpowlin/EBEB/toys.root --bias-labels invpowlin --bias-files $inputdir/bias_study_toys_from_mc_unbinned_moddijet/EBEB/toys.root --bias-labels moddijet -O $www --legend-labels --saveas pdf,convert_png,root --dont-draw --analyze-categories dijet --alternative-bias
...
#     www=~/www/exo/spring17/$1/bias_study_data_EBEE/${model}/
    ...
    # ./bkg_bias.py --read-ws $inputdir/bias_study_toys_from_mc_unbinned_nopureweight_${model}_asimov_EBEE.root  -o $inputdir/bias_study_toys_from_mc_unbinned_EBEE_asimov_fit_nopureweight_${model}.root --observable 'mgg[1934,330,10000]' --n-toys -1 -O $www --fit-range 330,5000 --use-data --models ${model} --fit-toys --fit-name cic2 --plot-binning 89,330,2110  --saveas root,png,pdf --verbose --components ""  --plot-toys-fits

    # ./bkg_bias.py --throw-toys --lumi-factor=1. --n-toys=1000 --components "" --models $model --fit-name cic2 --read-ws $inputdir/bias_study_input_nopureweight.root -o $inputdir/bias_study_toys_from_mc_unbinned_${model}_EBEE.root  -O $www --observable 'mgg[1934,330,10000]' --test-categories EBEE --use-data --fit-range 330,10000 --throw-from-model
    
    # ./submit_toys.sh 1nh $inputdir/bias_study_toys_from_mc_unbinned_${model}_EBEE.root  $inputdir/bias_study_toys_from_mc_unbinned_${model}/EBEE 1000 10 --observable 'mgg[1934,330,10000]' --fit-range 330,10000 
